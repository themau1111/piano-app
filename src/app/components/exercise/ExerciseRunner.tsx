...
export function ExerciseRunner({
  ...
  const scaleNoteResults = new Map((run?.feedback?.noteResults ?? []).map(({ midi, correct }) => [midi, correct]));
  const selectedStaffNotes = normalizeSelection(selected).map((midi) => ({
    midi,
    ...(run?.prompt.kind === "scale_construction" && scaleNoteResults.has(midi)
      ? { result: scaleNoteResults.get(midi) ? "correct" as const : "incorrect" as const }
      : {}),
  }));
