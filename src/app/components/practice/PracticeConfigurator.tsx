...
export function PracticeConfigurator({ exercise, locale, renderRunner }: { exercise: ExerciseDetail; locale: NotationLocale; renderRunner: (practice: Practice) => ReactNode }) {
  ...
  if (started) return <main className="mx-auto max-w-5xl px-4 py-8">{renderRunner({
    ...
    config: {
      constraints: {
        ...
        ...(exercise.kind === "scale_construction" ? { roots } : {}),
        ...(exercise.kind === "chord_identification" ? { qualities } : {}),
      },
      presentation: { showStaff },
    },
  })}</main>;
  ...
  return <main className="mx-auto max-w-3xl px-4 py-10 text-white">
    <section className="rounded-[28px] border border-white/10 bg-white/5 p-6">
      ...
      <h1 className="mt-2 text-3xl font-semibold">{exercise.title}</h1>
      ...
      <div className="mt-6 space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm">Preguntas<select value={count} onChange={(event) => setCount(Number(event.target.value))} className="mt-2 block w-full rounded-xl border border-white/15 bg-slate-900 px-3 py-2">{[5, 10, 15, 20].map((value) => <option key={value} value={value}>{value} preguntas</option>)}</select></label><label className="text-sm"><span className="flex items-center gap-2"><input type="checkbox" checked={timed} onChange={(event) => setTimed(event.target.checked)} /> Tiempo por pregunta</span>{timed && <input type="number" min="5" max="600" value={seconds} onChange={(event) => setSeconds(Number(event.target.value))} className="mt-2 block w-full rounded-xl border border-white/15 bg-slate-900 px-3 py-2" />}</label></div>
      </div>
